...
pub fn load_data() -> (Vec<Education>, Vec<Personality>) {
    let educations: Vec<Education> =
        serde_json::from_str(EDUCATION_FILE).expect("error while parsing education data");

    let personnalities: Vec<Personality> =
        serde_json::from_str(PERSONNALITIES_FILE).expect("error while parsing personality data");
...
pub fn remove_personnality(
    ...
    traits_incompatibles.into_iter().for_each(|value| {
        if let Some(index) = personality_bonus.iter().position(|pers| pers.name == value) {
            personality_bonus.remove(index);
        };

        if let Some(index) = personality_neutral
            .iter()
            .position(|pers| pers.name == value)
        {
...
pub fn generate_personnage(parameters: Parameters) -> Personnage {
        ...
        match parameters.education.as_ref().unwrap().as_str() {
            "diplomatie" | "martialite" | "intrigue" | "intendance" | "erudition" => {
                if age < Age(16) && age > Age(2) {
                    ...
                    educs_possible = educs_possible
                        .into_iter()
                        .filter(|educ| educ.level == 0 && educ.name == education_choosen)
                        .collect();
                } else if age >= Age(16) {
                    let education_choosen = parameters.education.unwrap();
                    educs_possible = educs_possible
    ...
    if education_level_is_some && age >= Age(16) {
        match parameters.level.unwrap() {
            1 | 2 | 3 | 4 | 5 => {
                ...
                educs_possible = educs_possible
                    .into_iter()
                    .filter(|educ| educ.level == education_level_choosen)
                    .collect();
            }
            _ => {
    ...
    if age < Age(16) && age > Age(2) {
        ...
        if education_level_is_some || education_is_some {
            ...
            if percentage < 10 {
                let very_good_education: Vec<Education> = educs_possible
                    .into_iter()
                    .filter(|educ| educ.level == 5)
                    .collect();
                ...
                let good_education: Vec<Education> = educs_possible
                    .into_iter()
                    .filter(|educ| educ.level >= 3 && educ.level < 5)
                    .collect();
                let educ_index = rng.random_range(0..good_education.len());
                education_personnage = Some(good_education[educ_index].clone());
            } else {
                let education: Vec<Education> = educs_possible
                    .into_iter()
                    .filter(|educ| educ.level < 3)
                    .collect();
        ...
        for personnality in personalities.into_iter() {
            ...
            for bonus in personnality.bonus.iter() {
                if education_personnage.name == "martialite"
                    && (bonus.name == education_personnage.name || bonus.name == "prouesse")
                    && bonus.apttitudes > 0
                {
    ...
    while personnality_personnage.len() < limit_number_personnality {
        let traits_incompatibles = if let Some(_) = education_personnage
            && rng.random_range(0..100) < 60
        {
            define_personality(
                &mut personality_bonus,
                &mut personnality_personnage,
                &mut points_personnage,
            )
        } else {
            define_personality(
                &mut personality_neutral,
                &mut personnality_personnage,
                &mut points_personnage,
            )
        };
    ...
    if let Some(education_personnage) = education_personnage.clone() {
        stats = stats
            .into_iter()
            .filter(|name| *name != education_personnage.name)
            .collect();
    }
